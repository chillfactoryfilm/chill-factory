...
(function () {
  ...
  function setupWorkFilter() {
    ...
    function apply(cat) {
      ...
      // 60SD renders as the wall: 5-across, captionless, hover-reveal titles
      grid.classList.toggle("grid--five", cat === "60sd");
      // per-category intro above the grid (currently 60SD only)
      var sub = document.querySelector("[data-grid-sub]");
      if (sub) {
        sub.hidden = cat !== "60sd";
        sub.classList.toggle("grid-sub--wall", cat === "60sd");
      }
  ...
  function catBarHTML(p) {
    ...
    // allOnly entries live on the ALL grid exclusively — light no category
    var own = p.allOnly ? null : workCat(p);
